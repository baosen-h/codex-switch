...
这个项目并不是一个成熟的软件项目。它是我在实际使用 Codex、Claude Code 和 Gemini 的过程中，为了让 Provider 配置、对话、绘图和 session 管理更方便而做出来的工具。

项目代码完全由 Codex 根据我的想法、需求描述、截图反馈和测试结果生成与修改。我主要负责提出需求、判断效果、测试功能，以及决定哪些地方需要继续调整。

因为开发时间不长，而且代码是这样一步步迭代出来的，所以它很可能还有 bug，也可能有一些逻辑写得不够好、文件组织不够清楚、实现方式不够优雅的地方。

我会继续在自己的使用中改进它。如果你也使用这个项目，并发现了问题，或者有更好的建议，欢迎提交 Issue。真实的反馈会很有帮助。
