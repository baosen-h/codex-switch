...
- 能力：发现、检查、安装并同步 Codex、Claude Code 和 Gemini 的技能与 MCP 服务。市场来源可配置，凭据存入操作系统密钥链，安装版本固定，只有用户明确操作时才更新。
...
<table>
  <tr>
    <th align="center">输入图片</th>
    <th align="center">Talking 结果</th>
  </tr>
  <tr>
    <td width="50%"><img src="docs/images/vision-source-image.png" alt="用于图片输入的浏览器截图" width="100%" /></td>
    <td width="50%"><img src="docs/images/vision-talking-result.png" alt="DeepSeek 在 Talking 页面中分析该图片" width="100%" /></td>
  </tr>
  <tr>
    <td align="center"><sub>用于测试图片理解能力的浏览器截图。</sub></td>
    <td align="center"><sub>纯文本 DeepSeek 模型在 Talking 中分析该图片。</sub></td>
  </tr>
  <tr>
    <th align="center">Codex CLI 请求</th>
    <th align="center">Codex CLI 结果</th>
  </tr>
  <tr>
    <td width="50%"><img src="docs/images/vision-codex-cli-request.png" alt="在 Codex CLI 的 DeepSeek 请求中附加图片" width="100%" /></td>
    <td width="50%"><img src="docs/images/vision-codex-cli-result.png" alt="DeepSeek 在 Codex CLI 中分析附加的图片" width="100%" /></td>
  </tr>
  <tr>
    <td align="center"><sub>在 Codex CLI 中附加并发送同一张图片。</sub></td>
    <td align="center"><sub>DeepSeek 获得视觉上下文后，返回能够理解图片内容的回答。</sub></td>
  </tr>
</table>
...
在**设置 → 自动联网搜索**中配置 Tavily、智谱、Exa、博查、SearXNG 或 Jina。
使用 Chat Completions 兼容接口的模型会自动获得 `web__search` 和 `web__fetch` 工具，
并自行判断何时需要最新信息，不需要用户在聊天中切换搜索模式。网页正文抓取可使用
内置直连或 Jina Reader。模型原生联网可用时仍优先使用原生能力。

直接读取网页时会逐次检查重定向、阻止私有和保留网络地址、只接受可读文本格式，并将
响应限制为 10 MB。搜索结果带有连续编号，模型可以在最终回答中生成 `[N]` 引用。
