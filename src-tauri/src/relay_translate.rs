...
fn transform_payload(payload: &mut Value, model_after_rewrite: &str) {
    ...
    let is_deepseek = is_deepseek_model(model_after_rewrite);
    ...
    if is_mimo {
        apply_mimo_quirks(obj, model_after_rewrite);
    }
    if is_deepseek {
...
fn apply_mimo_quirks(obj: &mut Map<String, Value>, model: &str) {
    if obj
        .get("tool_choice")
    ...
    if model != "mimo-v2-flash" && !obj.contains_key("thinking") {
        obj.insert(
            "thinking".to_string(),
            json!({
                "type": "enabled"
...
mod tests {
    ...
    fn sse_reasoning_content_stream() {
        ...
        assert!(contains_bytes(&all, b"thinking..."));
        assert!(contains_event(&all, b"response.completed"));
    }

    #[test]
    fn mimo_quirks_are_applied() {
        let codex = json!({
            "model": "gpt-5",
            "input": "hi",
        ...
        let (body, _state) =
            translate_request(&serde_json::to_vec(&codex).unwrap(), "mimo-v2-pro").unwrap();
        let v = parse(&body);
    ...
    #[test]
    fn deepseek_quirks_are_applied() {
        let codex = json!({
            "model": "gpt-5",
            "input": "hi",
            "tool_choice": {"type": "function", "name": "shell"},
    ...
    fn contains_bytes(blob: &[u8], needle: &[u8]) -> bool {
        blob.windows(needle.len()).any(|w| w == needle)
    }
